...
import { Play, RotateCcw, Flame, CheckCircle2, ChevronRight, Sparkles, Download, ShieldCheck } from 'lucide-react';
...
export const HomeView: React.FC<HomeViewProps> = ({
  ...
  const percentComplete = Math.round((user.today_reviewed / user.daily_goal) * 100);
    ...
    <div className="space-y-4">
      ...
      <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h4 className="text-xs font-bold text-stone-900 dark:text-stone-100">Full Project ZIP Ready</h4>
            <p className="text-[10px] text-stone-500 dark:text-stone-400">Worker + Supabase + TMA Frontend</p>
          </div>
        </div>
        <button
          id="btn-download-zip"
      ...
      <div className="bg-gradient-to-br from-emerald-900 via-emerald-950 to-stone-900 text-white p-5 rounded-3xl border border-emerald-800/40 shadow-sm relative overflow-hidden">
        ...
        <div className="flex items-center justify-between text-xs text-emerald-300 font-medium">
          <span>Assalamu Alaikum, {user.first_name}</span>
        ...
        <div className="mt-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-emerald-300/80 mt-0.5">
              {Math.max(0, user.daily_goal - user.today_reviewed)} verses due today
            </p>
          </div>
      ...
      {/* Primary Action Card: Continue Hifz */}
      ...
      <div className="bg-white dark:bg-stone-900 p-4 rounded-3xl border border-stone-200 dark:border-stone-800 shadow-sm">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-xs font-semibold text-stone-400">4 Ayahs Priority</span>
        </div>

        <div className="space-y-2">
          <div className="p-3 rounded-2xl bg-stone-50 dark:bg-stone-850 flex items-center justify-between text-xs">
            <div>
              <span className="font-bold text-stone-800 dark:text-stone-200">Al-Mulk : 1</span>
              <span className="ml-2 text-rose-500 font-semibold">Weak (Mistake logged)</span>
            </div>
            <button
              onClick={() => onNavigate('hifz')}
          ...
          <div className="p-3 rounded-2xl bg-stone-50 dark:bg-stone-850 flex items-center justify-between text-xs">
            <div>
              <span className="font-bold text-stone-800 dark:text-stone-200">Al-Mulk : 2</span>
              <span className="ml-2 text-amber-500 font-semibold">SRS Interval Due</span>
            </div>
            <button
              onClick={() => onNavigate('hifz')}
              className="text-emerald-600 dark:text-emerald-400 font-bold hover:underline flex items-center gap-0.5"
          ...
          <div className="p-3 rounded-2xl bg-stone-50 dark:bg-stone-850 flex items-center justify-between text-xs">
            <div>
              <span className="font-bold text-stone-800 dark:text-stone-200">Al-Fatihah : 7</span>
              <span className="ml-2 text-emerald-500 font-semibold">Retention Anchor</span>
            </div>
            <button
              onClick={() => onNavigate('hifz')}
              className="text-emerald-600 dark:text-emerald-400 font-bold hover:underline flex items-center gap-0.5"
      ...
      <div className="grid grid-cols-3 gap-2 text-center text-xs">
        ...
        >
          <span className="text-xl">🧩</span>
          <span className="font-semibold text-stone-800 dark:text-stone-200">Active Recall</span>
        </button>
        <button
        ...
        >
          <span className="text-xl">📖</span>
          <span className="font-semibold text-stone-800 dark:text-stone-200">Mushaf Reader</span>
        </button>
      </div>
      ...
      {!user.is_pro && (
        <div
          onClick={onOpenPro}
          className="cursor-pointer bg-gradient-to-r from-emerald-900 to-stone-900 p-4 rounded-3xl border border-emerald-700/40 text-white flex items-center justify-between shadow-sm"
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-emerald-200">
                Speech recognition, word-level diff, Telebirr & Stars
              </p>
            </div>
          </div>
          <span className="px-3 py-1 rounded-xl bg-amber-400 text-stone-950 font-extrabold text-xs">
            PRO
          </span>
        </div>
      )}

      {/* Inspiring Quran Verse */}
      <div className="text-center py-4 px-2 space-y-1">
        <p className="font-quran text-xl text-emerald-800 dark:text-emerald-400 leading-relaxed" dir="rtl">
